...
private:
  bool processLoop(MachineLoop *L, MachineFunction &MF, const TargetInstrInfo *TII);
  bool unrollLoop(MachineLoop *L, MachineFunction &MF, const TargetInstrInfo *TII, unsigned UnrollCount);
};

char FrolovaSLoopUnroll::ID = 0;

bool FrolovaSLoopUnroll::processLoop(MachineLoop *L, MachineFunction &MF, const TargetInstrInfo *TII) {
  bool Changed = false;
...
bool FrolovaSLoopUnroll::unrollLoop(MachineLoop *L, MachineFunction &MF, const TargetInstrInfo *TII, unsigned UnrollCount) {
  if (UnrollCount <= 1) return false;

  MachineBasicBlock *LoopMBB = L->getHeader();
  if (L->getNumBlocks() != 1) {
    llvm::outs() << "Skipping complex loop (multiple blocks) in " << MF.getName() << "\n";
    return false;
  }
...
bool FrolovaSLoopUnroll::runOnMachineFunction(MachineFunction &MF) {
  llvm::outs() << "Running ExamplePass on function: " << MF.getName() << '\n';
...
static RegisterPass<ExamplePass> X("example-x86", "FrolovaSLoopUnrollPass", false, false);
