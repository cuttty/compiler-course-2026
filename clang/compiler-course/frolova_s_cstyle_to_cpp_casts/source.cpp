...
class CastRewriterVisitor final : public clang::RecursiveASTVisitor<CastRewriterVisitor> {
public:
    ...
    bool VisitCStyleCastExpr(clang::CStyleCastExpr *Node) {
        ...
        if (!SM.isInMainFile(Node->getBeginLoc()) || Node->getBeginLoc().isMacroID())
            return true;
...
if (Kind == clang::CK_BitCast || Kind == clang::CK_LValueBitCast) {
    ...
    CastName = "const_cast";
        ...
        clang::SourceLocation SubExprLoc = Node->getSubExprAsWritten()->getBeginLoc();
        ...
        clang::SourceRange CastRange(Node->getBeginLoc(), SubExprLoc.getLocWithOffset(-1));
...
public:
    ...
    void HandleTranslationUnit(clang::ASTContext &Context) override {
        ...
        Rewrite.getEditBuffer(CI.getSourceManager().getMainFileID()).write(llvm::outs());
    }
...
static clang::FrontendPluginRegistry::Add<CastAction> 
    X("cstyle_cast_replacer", "Replace C-style casts with C++ casts and rewrite code");